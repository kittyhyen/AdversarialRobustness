...
# %%
parser = argparse.ArgumentParser('Adversarial Logit Pairing with Various Aug')
...
save_name = "logit_pairing_%g_aug_%s" % (
...
def main():
    ...
    for epoch in range(args.epoch):
        ...
        for data in train:
            ...
            total_loss = ce_loss + args.logit_pairing_lambda * logit_pairing_loss

            # test_str += "%.8f  " % proposed_loss.item()
            #print(ce_loss.item(), logit_pairing_loss.item())
        ...
        print_str = "[Epoch %3d] CELoss: %.8f  LogitLoss: %.8f  " % (
            epoch + 1, eval_dict['TrainCELoss'], eval_dict['TrainLogitLoss']
        )
