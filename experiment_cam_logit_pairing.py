...
from lib.iccc_util import wide_cam_pre_act_resnet32
...
# %%
parser = argparse.ArgumentParser('CAM & Logit Pairing ADT with Various Aug')
...
parser.add_argument('--cam_pairing_lambda', type=float, default=5.)
parser.add_argument('--logit_pairing_lambda', type=float, default=1.)
...
save_name = "cam_logit_pairing_%g_%g_aug_%s" % (
    args.cam_pairing_lambda,
    args.logit_pairing_lambda,
    args.augment
)
...
def main():
    model = wide_cam_pre_act_resnet32(width=args.model_width, input_standardize=args.input_std, num_classes=num_classes)
    ...
    for epoch in range(args.epoch):
        ...
        total_iter = 0
        total_ce_loss = 0.
        total_cam_pairing_loss = 0.
        total_logit_pairing_loss = 0.
        ...
        for data in train:
            ...
            # loss         #########################################################
            ce_loss = smoothing_criterion(torch.cat((white_outputs, outputs), dim=0),
                                          torch.cat((labels, labels), dim=0))
            cam_pairing_loss = pairing_criterion(model.forward_conv_to_cam_all(white_conv, normalize_type=None),
                                                 model.forward_conv_to_cam_all(conv, normalize_type=None))
            logit_pairing_loss = torch.mean(torch.sum(torch.pow(outputs - white_outputs, 2), dim=1), dim=0)
            total_loss = ce_loss + args.cam_pairing_lambda * cam_pairing_loss + args.logit_pairing_lambda * logit_pairing_loss

            # test_str += "%.8f  " % proposed_loss.item()
            #print(ce_loss.item(), cam_pairing_loss.item())
            ...
            total_cam_pairing_loss += cam_pairing_loss.item()
            total_logit_pairing_loss += logit_pairing_loss.item()
            correct += outputs.max(1)[1].eq(labels).sum().item()
            white_correct += white_outputs.max(1)[1].eq(labels).sum().item()

        eval_dict['TrainCELoss'] = total_ce_loss / total_iter
        eval_dict['TrainCAMLoss'] = total_cam_pairing_loss / total_iter
        eval_dict['TrainLogitLoss'] = total_logit_pairing_loss / total_iter
        ...
        print_str = "[Epoch %3d] CELoss: %.8f  CAMLoss: %.8f  LogitLoss: %.8f  " % (
            epoch + 1, eval_dict['TrainCELoss'], eval_dict['TrainCAMLoss'], eval_dict['TrainLogitLoss']
        )
