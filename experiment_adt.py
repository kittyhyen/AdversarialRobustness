...
from lib.resnet_madry import wide_pre_act_resnet32
...
from lib.mixup_util import MixUpGenerator_v1

# %%
parser = argparse.ArgumentParser('ADT with Various Aug')
parser.add_argument('--gpu', type=int, default=0, choices=[0, 1, 2, 3])

# training setup
parser.add_argument('--dataset', type=str, default='tiny', choices=['cifar10', 'cifar100', 'stl10', 'tiny'])
...
parser.add_argument('--lr_decay_epoch', type=str, default="20,30")
...
# model setup
parser.add_argument('--model_width', type=int, default=10, help='model width for wide resnet')
...
parser.add_argument('--with_mixup', action='store_true')

# logging setup
parser.add_argument('--save', action='store_true', help='save model state & eval log')
parser.add_argument('--save_model_term', type=int, default=40)
...
save_name = "adt_aug_%s" % (
    args.augment
)
if args.with_mixup:
    save_name += "_with_mixup"
...
def main():
    model = wide_pre_act_resnet32(width=args.model_width, input_standardize=args.input_std, num_classes=num_classes)
    ...
    multi_rs_pgd_eval = MultiRandomStartPGDEval(model,
                                                ...
                                                num_steps=args.pgd_num_steps,
                                                step_size=args.pgd_step_size / 255)
    mixup_gen = MixUpGenerator_v1(prob=1., scale=(0.8, 1.0))
    ...
    for epoch in range(args.epoch):
        ...
        for data in train:
            ...
            if args.with_mixup:
                mixup_inputs = mixup_gen.generate(inputs)
                white_inputs = pgd.generate(mixup_inputs, y=labels)
            else:
                white_inputs = pgd.generate(inputs, y=labels)

            white_outputs = model(white_inputs)
        ...
        print_str = "[Epoch %3d] CELoss: %.8f  " % (
            epoch + 1, eval_dict['TrainCELoss'],
        )
